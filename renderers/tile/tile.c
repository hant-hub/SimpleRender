...
ErrorCode TileInit(TileRenderer* r, RenderPass* p, const char* tileset, u32 subpass, sm_vec2i tilesize, sm_vec2i mapsize) {
    ...
    TileLoadSetFile(&r->tileset, tileset, tilesize.x, tilesize.y);
    r->mapsize = mapsize;

    for (int i = 0; i < SR_MAX_FRAMES_IN_FLIGHT; i++) {
        PASS_CALL(CreateDynamicBuffer(mapsize.x * mapsize.y * sizeof(int), &r->tiledata[i], VK_BUFFER_USAGE_STORAGE_BUFFER_BIT));
...
void TileSetData(TileRenderer* r, int* data) {
    for (int i = 0; i < SR_MAX_FRAMES_IN_FLIGHT; i++) {
        memcpy(r->tiledata[i].buffer, data, r->mapsize.x * r->mapsize.y * sizeof(int));
    }
}

void TileSetRow(TileRenderer* r, int* data, u32 row) {
    u32 offset = row * r->mapsize.x * sizeof(int);
    for (int i = 0; i < SR_MAX_FRAMES_IN_FLIGHT; i++) {
        memcpy(r->tiledata[i].buffer + offset, data, r->mapsize.x * sizeof(int));
    }
}

void TileSetColumn(TileRenderer* r, int* data, u32 col) {
}

void TileSetTile(TileRenderer* r, int data, sm_vec2i pos) {
}
...
void TileDrawFrame(TileRenderer* r, PresentInfo* p, u32 frame) {
    Uniform ub = (Uniform) {
        ...
        .mapsize = (sm_vec2f){r->mapsize.x, r->mapsize.y},
